...
export const InfoBannerCard: React.FC<InfoBannerCardProps> = ({
  ...
  return (
    <div className="w-full py-1">
      ...
      {/* When inactive or no active items, show Master control alert if Master User */}
      {!announcement.isActive || activeItems.length === 0 ? (
        ...
        <div className="bg-white border border-slate-200/90 rounded-xl px-3 py-2 sm:px-3.5 sm:py-2.5 shadow-sm text-slate-800 flex items-center gap-2.5 sm:gap-3 overflow-hidden relative">
          ...
          >
            <div
              className="animate-marquee font-bold text-xs sm:text-sm text-slate-800 tracking-wide flex items-center whitespace-nowrap select-none"
              style={{
          ...
          {isMasterUser && (
            <button
              type="button"
              onClick={handleOpenEditModal}
              className="shrink-0 p-1.5 bg-slate-100 hover:bg-amber-100 text-slate-700 hover:text-amber-900 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center gap-1 border border-slate-200/80"

              ...
              <div className="text-slate-800 animate-fadeIn">
                ...
                {activeTab === 'login' && (
                  <div className={isMasterDataLogin ? "grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start animate-fadeIn" : ""}>
                    ...
                    {/* Google Login Button - Hidden for Student Login */}
                    {!isStudentLogin && (
                      <button
                        type="button"
                        disabled={isLoading}
                        onClick={handleGoogleSignIn}
                        className="w-full mt-3 py-3.5 bg-white border border-slate-300 text-slate-700 font-bold text-sm sm:text-base rounded-xl shadow-sm hover:bg-slate-50 flex items-center justify-center space-x-3 transition-all active:scale-[0.99] cursor-pointer disabled:opacity-50"
                      >
